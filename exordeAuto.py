...
def switch(ip, un, pw):
    try:
        ...
        client.connect(ip, username=un, password=pw, key_filename='/var/www/html/exorde/assets/pem/'+paramUn+'.pem')
        if sys.argv[4] == "1":
            ...
            commandExec = "sudo docker stop $(docker ps -qf 'name=^exorde') && sudo docker rm $(docker ps -a -qf 'name=^exorde')"
            # print(commandExec)
            stdin, stdout, stderr = client.exec_command(commandExec)
            stdout=stdout.readlines()
            stdin.close()
